...
const SITE_DIR='/home/tawfeeq/ramadan-clock-site',PORT=9941,FPS=30,DURATION=15,TOTAL=FPS*DURATION;
...
async function main(){
  ...
  await page.evaluate(async()=>{
    ...
    // Pill + label at 12% from bottom (safe zone)
    ...
    s.style.cssText='position:fixed;top:calc(12% + 3.8rem);left:0;width:100%;text-align:center;z-index:9999;pointer-events:none;font-family:"Instrument Serif",serif;font-size:0.85rem;font-weight:400;color:rgba(232,228,220,0.4);padding:0 15%';
  ...
  for(let i=0;i<TOTAL;i++){
    ...
    await page.evaluate((s,ms)=>{window._fakeTime.s=s;window._fakeTime.ms=ms},s,ms);
    await new Promise(r=>setTimeout(r,50));
    const frame=String(i).padStart(4,'0');
    await page.screenshot({path:OUT_DIR+'/frame_'+frame+'.png',type:'png'});
    if(i%45===0) console.log('  Frame '+i+'/'+TOTAL+' ('+s+'s)');
  }
  ...
  // 9:16 from 1290x2796: width stays 1290, height = 1290*16/9 = 2293. Crop center.
  execSync('ffmpeg -y -framerate '+FPS+' -i "'+OUT_DIR+'/frame_%04d.png" -c:v libx264 -crf 18 -pix_fmt yuv420p -vf "crop=1290:2293:0:(2796-2293)/2,scale=trunc(iw/2)*2:trunc(ih/2)*2" /home/openclaw-agent/.openclaw/workspace/poster-lastthird-video-916.mp4');

  const s1=fs.statSync('/home/openclaw-agent/.openclaw/workspace/poster-lastthird-video.mp4');
  const s2=fs.statSync('/home/openclaw-agent/.openclaw/workspace/poster-lastthird-video-916.mp4');
  console.log('Done!');
