...
const OUT = path.join(__dirname, 'night-frames-v4');
const FRAMES = 600;
const OUTPUT_MP4 = path.join(__dirname, 'agot-timelapse-night-v4.mp4');
...
async function run() {
  ...
  // Hide chrome + inject title + lock second hand
  await page.evaluate(() => {
    // Hide all chrome
    ...
    // Title overlay — EXACT info page styling
    const title = document.createElement('div');
    title.textContent = 'A Gift of Time.';
    title.style.cssText = 'position:fixed;top:22%;left:0;right:0;text-align:center;font-family:inherit;font-size:clamp(2.2rem,6vw,3.8rem);font-weight:200;letter-spacing:-0.02em;color:rgba(232,228,220,0.9);z-index:9999;pointer-events:none;line-height:1;';
  ...
  const testPath = path.join(__dirname, 'night-v4-test.png');
