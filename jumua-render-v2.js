...
const FRAMES = 450;
const FPS = 30;
const FRAME_DIR = '/tmp/jumua-frames-v3';
...
(async () => {
  ...
  // Pin forceTimeMin to Dhuhr
  await p.evaluate(() => {
    window._forceTimePin = setInterval(() => { window._forceTimeMin = 755; }, 100);
  });
  await new Promise(r => setTimeout(r, 3000));

  // Override _cityNow to return a fake date that auto-advances at real-time speed
  // but starts at 12:35:00
  await p.evaluate(() => {
    var startReal = performance.now();
    var startFake = new Date(2026, 2, 13, 12, 35, 0, 0).getTime(); // March 13, 12:35:00.000

    // Override _cityNow directly — this is what the animation loop uses
    window._cityNow = function() {
      var elapsed = performance.now() - startReal; // ms since we started
      return new Date(startFake + elapsed); // advance fake time at same rate
    };
  });
