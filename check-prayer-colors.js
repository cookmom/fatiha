...
const SITE_DIR='/home/tawfeeq/ramadan-clock-site',PORT=9933;
...
async function main(){
  ...
  await page.evaluateOnNewDocument(()=>{window._forceTimeMin=0;window._forceLocation={lat:21.4225,lon:39.8262,name:'Makkah'}});
  await page.goto(`http://localhost:${PORT}/`,{waitUntil:'domcontentloaded',timeout:30000});
  await new Promise(r=>setTimeout(r,12000));

  for(const min of testMins) {
    await page.evaluate((m)=>{window._forceTimeMin=m},min);
    await new Promise(r=>setTimeout(r,2000)); // let it settle
    
    const info = await page.evaluate(()=>{
      const root=document.documentElement;
      return {
        glowBar: root.style.getPropertyValue('--pill-glow-bar'),
        glowShadow: root.style.getPropertyValue('--pill-glow-bar-shadow'),
        pillGlow: root.style.getPropertyValue('--pill-glow'),
      };
    });
    console.log(`min=${String(min).padStart(4)}: glow=${info.glowBar||'(default #e89830)'} shadow=${info.glowShadow||'default'}`);
  }
