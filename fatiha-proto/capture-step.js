...
const targetFrame = parseInt(process.argv[2] || '120');
const outName = process.argv[3] || `step-f${targetFrame}.png`;
...
(async () => {
  ...
  console.log(`Capturing frame ${targetFrame} -> ${outName}`);
  ...
  console.log('p5 running, advancing...');
  ...
  await page.screenshot({ path: path.join(RENDER_DIR, outName), fullPage: false });
  console.log(`Saved: renders/${outName}`);
