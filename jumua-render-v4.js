...
const FRAMES = 450;
const FRAME_DIR = '/tmp/jumua-frames-v5';
const MS_PER_FRAME = 1000 / 30; // 33.333ms
...
(async () => {
  ...
  // Controlled clock: only advances when we tell it to
  await p.evaluate(() => {
    window._fakeMs = 0;
  ...
  for (let i = 0; i < FRAMES; i++) {
    // Advance fake clock by exactly 1/30th of a second
    await p.evaluate((ms) => { window._fakeMs += ms; }, MS_PER_FRAME);
    
    // Wait one rAF tick so the scene re-renders with new time
