...
const SITE_DIR='/home/tawfeeq/ramadan-clock-site',PORT=9934;
...
const PRAYER_COLORS = {
  ...
  '#ff2200':'Maghrib', '#1166ff':'Isha', '#e89830':'default(orange)'
};
...
async function main(){
  ...
  const testMins = [0, 30, 90, 151, 230, 300, 350, 400, 540, 750, 850, 953, 1030, 1109, 1170, 1229, 1300, 1400];
  
  // Load once, then change forceTimeMin and wait for update
  const page=await browser.newPage();
  await page.setViewport({width:430,height:932,deviceScaleFactor:1});
  await page.emulateTimezone('Asia/Riyadh');
  await page.evaluateOnNewDocument(()=>{
    window._forceTimeMin=0;
  ...
  for(const min of testMins) {
    ...
    await new Promise(r=>setTimeout(r,3000)); // wait for render cycle to pick it up
    ...
    const info = await page.evaluate(()=>{
      ...
      return {
        glowBar,
        activePrayerColor: window._activePrayerColor||'none',
      };
    });
    const colorName = PRAYER_COLORS[info.glowBar] || PRAYER_COLORS[info.activePrayerColor] || `unknown(${info.glowBar}/${info.activePrayerColor})`;
    console.log(`min=${String(min).padStart(4)} (${String(Math.floor(min/60)).padStart(2)}:${String(min%60).padStart(2,'0')}): color=${info.activePrayerColor||info.glowBar} → ${colorName}`);
  }
