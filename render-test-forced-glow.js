...
const SITE_DIR='/home/tawfeeq/ramadan-clock-site',PORT=9938;
...
async function main(){
  ...
  // Force pill glow to match active prayer color
  await page.evaluate(()=>{
    const gc = window._activePrayerColor || '#aa44ff';
  ...
  const pillColor = await page.evaluate(()=>{
    const s=document.getElementById('modePillSlider');
    return s?s.style.getPropertyValue('--pill-glow-bar'):'none';
  });
  console.log('Pill glow bar:', pillColor);
  ...
  await page.screenshot({path:'/home/openclaw-agent/.openclaw/workspace/test-lastthird-forced.png',type:'png'});
  console.log('Done');
