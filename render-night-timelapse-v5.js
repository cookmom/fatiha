...
const OUT = path.join(__dirname, 'night-frames-v5');
const FRAMES = 600;
const OUTPUT_MP4 = path.join(__dirname, 'agot-timelapse-night-v5.mp4');
const TEST_SCREENSHOT = path.join(__dirname, 'timelapse-v5-test.png');
...
async function run() {
  ...
  // Hide chrome + inject LARGE BOLD title + lock second hand
  await page.evaluate(() => {
    // Hide all chrome
    ...
    // Title overlay — TRIPLE SIZE, BOLD
    const title = document.createElement('div');
    title.textContent = 'A Gift of Time.';
    title.style.cssText = 'position:fixed;top:12%;left:0;right:0;text-align:center;font-family:inherit;font-size:clamp(6.6rem,18vw,11.4rem);font-weight:700;letter-spacing:-0.02em;color:rgba(232,228,220,0.9);z-index:9999;pointer-events:none;line-height:1;';
